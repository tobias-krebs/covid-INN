...
class CubicSplineBlock(InvertibleModule):
    ...
    def __init__(self, dims_in, dims_c=[],
        ...
        self.bounds_init = bounds_init
        self.bounds = torch.ones(1, self.splits[1], *([1] * self.input_rank)) * float(bounds_init)
        self.tails = tails
    ...
    def _unconstrained_cubic_spline(self,
        ...
        if len(inputs)==0:
            print("skipped some blocks",flush=True)
            return masked_outputs, masked_logabsdet

        bound = self.bounds_init
        left = -bound
        right = bound
        bottom = -bound
        top = bound
        ...
        if rev:
            ...
            discriminant[discriminant==0]=eps
